...
router.get('/api/walkers/summary', async (req, res) => {
  try {
    const [walker] = await db.execute(`SELECT Users.username AS walker_username, COUNT(WalkRatings.walker_id) AS total_rating, AVG(WalkRating.rating) AS average_rating, COUNT(CASE WHEN WalkRequests.status = 'completed' THEN 1 END) AS completed_walks FROM Users INNER JOIN WalkRatings ON Users.user_id = walker_id LEFT JOIN WalkRequests ON WalkRatings.request_id = WalkRequests.request_id WHERE Users.role = 'walker' GROUP BY Users.user_id;`);
